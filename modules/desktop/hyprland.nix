  ...
  flake.nixosModules.core = {
    ...
    hjem.users.${user}.rum.programs.hyprland = {
      ...
      settings = {
        ...
        workspace = lib.optionals (lib.length config.preferences.monitors > 1) (builtins.concatMap (
            monitor:
              builtins.map (workspace: "${toString workspace}, monitor:${monitor.name}") monitor.workspaces
          )
          config.preferences.monitors);

        monitor = lib.forEach config.preferences.monitors (
          monitor: "${monitor.name}, ${monitor.resolution}@${toString monitor.refreshRate}, ${monitor.position}, auto"
        );
      };
    };
